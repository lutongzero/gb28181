...
public class SipLayerConfiguation {
	...
	public Address createAddress(AddressFactory addressFactory, SipServerInfo sipinfo) throws ParseException {
		SipURI from = addressFactory.createSipURI(sipinfo.getId(), sipinfo.getIp() + ":" + sipinfo.getPort());
	...
	@Bean
	@Scope("prototype")
	public MaxForwardsHeader getDefaultMaxForwardsHeader(HeaderFactory headerFactory) throws InvalidArgumentException {
	...
	public Map<String, SipProvider> createSipProvider(SipServerInfo info) throws TooManyListenersException,
			TransportNotSupportedException, InvalidArgumentException, PeerUnavailableException, ObjectInUseException {
		...
		log.info("sip ip{},端口号 {}", info.getIp(), info.getPort());
		return map;
	}
